...
class FLSTM_RE_Pipeline:
    ...
    def __Verify_RE_PARAMS (self):
        ...
        if (self.__RE_PARAMS.DevelopmentSet_Files_List == None) or (self.__RE_PARAMS.DevelopmentSet_Files_List == []):
            self.__PRJ.lp ("No validation __DATAset is given. Hence " + str (self.__RE_PARAMS.KERAS_validation_split_IfDevel__DATANotGiven * 100) + "% of Training __DATA will be used for validation"); 
    ...
    def __PrepareNetworkInput__DATA (self):
        ...
        for idx , L in enumerate([TrainingSet_Files_List, DevelopmentSet_Files_List, TestSet_Files_Lists]):
            ...
            for FileAddress in L:
                Sentences , Root , LOCAL_TotalCorpusRootAnalysisResult , docs, docs_info  = self.__PRJ.Preprocessor.OnTheFlyPipeline (FileAddress,DuplicationRemovalPolicy); 
    ...
    def __WriteBackTestSetPredictionResults (self, y_pred, confs, PAIR_TRACKING, GOLD_XML_FileAddress, OutputFileAddress):
    ...
    def __HandleTestSet (self):
        if self.__RE_PARAMS.PredictTestSet:
            ...
            if self.__RE_PARAMS.WriteBackTestSetPredictions: 
                ...
                #{END}<<<CRITICAL>>>
                self.__WriteBackTestSetPredictionResults (y_pred["y_vector"], confs["y_vector"], PAIR_TRACKING, GOLD_XML_FileAddress, OutputFileAddressName);
                ...
                if ("y_vector_lincomb" in y_pred):
                    ...
                    self.__WriteBackTestSetPredictionResults (y_pred["y_vector_lincomb"], confs["y_vector_lincomb"], PAIR_TRACKING, GOLD_XML_FileAddress, OutputFileAddressName);
