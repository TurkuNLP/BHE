...
def process(fn, mapping):
    ...
    print "rewriting unicode --> ascii" 
    new_text, new_idx = replace_mapped(root, mapping)
    ...
    if output_fn == fn and not options.overwrite:
        ...
        try:
            with open(output_fn, 'w') as of:
                of.write(new_text)            
            with open(output_fn + '.idx', 'w') as of:
                of.write(new_idx)
        except IOError, ex:
...
def argparser():
    import argparse
    ap=argparse.ArgumentParser(description='Rewrite Unicode text content with approximately equivalent ASCII in normal text files.')
