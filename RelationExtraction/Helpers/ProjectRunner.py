...
import NetworkInputGeneration as NetworkInputGenerator ; 
...
class FLSTM_ProjectRunner:
    def __init__ (self, PARAM_ConfigFileAddress , PARAM_LogFileAddress):
        ...
        self.wv = GF.LoadW2VModelFromFile (self.lp , self.PROGRAM_Halt , self.Configs);
        self.Preprocessor           = Preprocessor.FLSTM_Preprocessor (self.lp , self.PROGRAM_Halt , self.Configs , self.wv);
        self.NetworkInputGenerator  = NetworkInputGenerator.FLSTM_NetworkInputGenerator (self.lp , self.PROGRAM_Halt , self.Configs , self.wv); 
    ...
    def ConfigFileVerification (self):
        ...
        if not "TotalWords" in self.Configs["W2V_Model"]:
            self.PROGRAM_Halt ("Missing TotalWords in W2V_Model dictionary in config file.");
